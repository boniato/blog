...
public class CategoryControllerTest {
	...
	public void categories() throws Exception {
		...
		List<Category> categories = Arrays.asList(new Category(1L, "spring"), new Category(2L, "jpa"),
				new Category(3L, "java"), new Category(4L, "spring-boot"), new Category(5L, "javascript"));

		Page<Category> categoryPage = new PageImpl<>(categories);

		given(this.categoryService.findAll(anyObject())).willReturn(categoryPage);
		MvcResult mvcResult = this.mvc.perform(get("/categories")).andExpect(status().isOk()).andReturn();
	...
	public void newCategory() throws Exception {
		this.mvc.perform(get("/categories/new")).andExpect(status().isOk()).andExpect(view().name("category/new"))
				.andReturn();
	}
	...
	public void edit() throws Exception {
		...
		MvcResult mvcResult = this.mvc.perform(get("/categories/{id}/edit", 1)).andExpect(status().isOk()).andReturn();
	...
	public void createCategory() throws Exception {
		...
		this.mvc.perform(post("/categories").param("name", "spring")).andExpect(status().isFound())
	...
	public void modifyCategory() throws Exception {
		...
		this.mvc.perform(post("/categories/{id}/edit", 1L).param("name", "spring-boot")).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, "/categories"));
	}
	...
	public void deleteCategory() throws Exception {
		...
		this.mvc.perform(post("/categories/{id}/delete", 1L)).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, "/categories"));
	}
}